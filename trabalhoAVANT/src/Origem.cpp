...
// main() function
// ----------------------------------------------------------
vector<double> pegarpontos(int argc, char* argv[]) {
    ...
    cout << "imprimindo o caminho" << endl;
      ...
      for (int i = 0; i < N.size(); i++) {	
        if (N[i].get_obs() == 1) {
            ...
            N[i].print_point();
	    //criar um vetor que armazena esses pontos
	    pontos.push_back(N[i].get_x());
	    pontos.push_back(N[i].get_y());
	    pontos.push_back(N[i].get_z());
            cout << endl;
        }
    }
