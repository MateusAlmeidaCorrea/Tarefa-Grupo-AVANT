...
int main(int argc, char **argv)
{
    ...
    //posicao inicial
    pose.pose.position.x = pontos_final[2]/15+0.5; //ajuste feito para o drone ir para o centro do quadrado
    pose.pose.position.y = pontos_final[1]/15-0.5;
    pose.pose.position.z = pontos_final[0]/15+3;//ajuste feito pois no codigo o drone inicia-se no meio do espaco e para esse codigo ele deve comecar na altura mais baixa
    for(int i = 100; ros::ok() && i > 0; --i){
      rate.sleep();
    }
    //preparando o drone para decolar
    mavros_msgs::SetMode offb_set_mode;
    offb_set_mode.request.custom_mode = "OFFBOSARD";
	...
	if(((pose.pose.position.x - 0.2) < gps.pose.position.x && gps.pose.position.x < (pose.pose.position.x + 0.2)) &&
	   ...
	   if (i < intervalo){
	     ...
	     i = i+3;
	     pose.pose.position.x = (pontos_final[i+2]/15+0.5);
	     pose.pose.position.y = (pontos_final[i+1]/15-0.5);
